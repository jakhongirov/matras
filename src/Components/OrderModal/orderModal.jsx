...
function OrderModal({ modal, setModal, pr, setPr }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (checkedUser !== null && checkedPhone.length === 9 && id.length) {
      const obj = {
        user_name: userName,
        user_phone: `+998 ${userPhone}`,
        order_pro_name: id,
        order_count: amount - 0,
      };

      fetch("http://localhost:7000/products/orders", {
        method: "POST",
                        ...
                        {allProduct
                          ? allProduct.map((p) => {
                            if (p.id === defValue) {
                              ...
                              return (
                                <option selected={true} value={p.pro_id} key={p.pro_id}>
                                  {p.pro_name}
                                </option>
                              );
                            }
                            return (
                              <option value={p.pro_id} key={p.pro_id}>
                                {p.pro_name}
                              </option>
                            );
                          })
